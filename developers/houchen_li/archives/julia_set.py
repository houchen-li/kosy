    ...
    def newton_raphson(
        ...
        *,
        ...
        if isinstance(z0, np.number):
            ...
            zs: npt.NDArray[T] = np.empty(shape=(z0.size,), dtype=z0.dtype)
            dzs: npt.NDArray[T] = np.empty(shape=(z0.size,), dtype=z0.dtype)
            iters: npt.NDArray[np.int64] = np.empty(shape=(z0.size,), dtype=np.int64)

            if num_workers == 1:
                for i, z in enumerate(np.ravel(z0)):
                    zs[i], dzs[i], iters[i] = self._newton_raphson(
                        z, abs_tol=abs_tol, rel_tol=rel_tol, max_iter=max_iter
                    )
            else:
                with ProcessPoolExecutor(max_workers=num_workers) as executor:
                    futures: list[Future[tuple[T, T, np.int64]]] = [
                        executor.submit(
                        ...
                        for z in np.ravel(z0)
                    ]
                    for i, future in enumerate(futures):
                        zs[i], dzs[i], iters[i] = future.result()

            return np.reshape(zs, z0.shape), np.reshape(dzs, z0.shape), np.reshape(iters, z0.shape)
        else:
            raise NotImplementedError(f"Unsupported type for z0: {type(z0)!r}")
...
def main() -> int:
    ...
    _zs, _dzs, iters = f.newton_raphson(z0, abs_tol=abs_tol, rel_tol=rel_tol, max_iter=max_iter)
