...
| ---- | ------------------------------- | -------------------------------------------------------------- | ----- | ------ |
| 1    | `alice_knn_nanoflann_vendor`    | `nanoflann/xmake.lua`, `nanoflann.hpp`                             | build | ~2,560 |
...
| 4    | `alice_knn_build_and_benchmark` | `xmake.lua`, `benchmark.cc`                                        | build | ~627   |
...
# PR #4: xmake.lua and benchmark (last — xmake references all others)
git diff $DIFF_BASE -- src/math/knn/xmake.lua \
...
git commit -m "Add KNN xmake.lua and benchmark"
git push -u origin alice_knn_build_and_benchmark
gh pr create --draft --title "build(knn): add xmake.lua and benchmark for KNN library" --base master --body ""
cd "${ORIGINAL_DIR}"
...
## Example 2: Evaluator Module Split (xmake.lua File Handling)

### Original PR

```
src/evaluator/xmake.lua                         |  50 +
...
| PR # | Branch                       | Source Files                                             | xmake targets                                         |
| ---- | ---------------------------- | -------------------------------------------------------- | ----------------------------------------------------- |
...
# Create xmake.lua with ONLY statistics_aggregator targets (Scenario B — new directory)
# See build-file-handling.md §Scenario B for the xmake.lua content
...
# Write PR-specific xmake.lua here
git add .
...
mkdir -p src/evaluator
# Write PR-specific xmake.lua here (see build-file-handling.md)
git add .
...
2. Update branch → merge `alice_evaluator_main` (xmake.lua now has all targets)
...
4. **Missing deps are OK** — `xmake.lua` files can reference targets that don't exist yet
5. **Existing vs new directory** — KEEP all existing xmake targets (Scenario A) vs create per-PR `xmake.lua` (Scenario B)
