...
| Category                              | Priority                          |
| ------------------------------------- | --------------------------------- |
| Infrastructure (xmake.lua, deps, configs) | Split first — often blocks others |
...
2. **For each changed header**, find ALL consumers in the repo:
   ```bash
   # Find xmake targets depending on the changed library
   grep -r 'target("<target_name>")' --include=xmake.lua <repo_path>
...
- **If independent builds are required** — Each PR must build and pass tests independently
  - May require duplicating `xmake.lua` files or shared configs across PRs

### Decision: xmake.lua file handling

- **Run** `git show origin/master:path/to/xmake.lua 2>/dev/null` to check if `xmake.lua` exists on master
- **If xmake.lua exists on master** → Scenario A: KEEP all existing targets, ADD new ones. **Read** [build-file-handling.md](build-file-handling.md) §Scenario A.
- **If no xmake.lua on master** → Scenario B: Create separate `xmake.lua` per split PR with only relevant targets. **Read** [build-file-handling.md](build-file-handling.md) §Scenario B.

⚠️ **Never replace an existing `xmake.lua` with only your new targets** — this deletes all existing targets. See [troubleshooting.md](troubleshooting.md) §BUILD Targets Missing.
...
**Handle `xmake.lua`** inside the worktree according to the decision in Phase 2. **Read** [build-file-handling.md](build-file-handling.md) for code templates.
...
- [build-file-handling.md](build-file-handling.md) — `xmake.lua` scenarios (existing vs new directory) with code templates
