...
```bash
# Python
uv sync                 # installs dependencies + builds the nanobind extension
uv run pytest
```
...
- **`CMAKE_UNITY_BUILD`**, **`CMAKE_CXX_EXTENSIONS`**: standard project toggles

The project is a Python package; all kosy libraries created via
`kosy_cxx_library` are either header-only INTERFACE targets or `SHARED`
libraries. There is no `BUILD_SHARED_LIBS` toggle.

## CMake functions

See `cmake/utils.cmake`:

- `kosy_cxx_library` — INTERFACE library when no SRCS, SHARED library otherwise
