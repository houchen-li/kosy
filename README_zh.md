...
# 烟雾测试
uv run python -c "from kosy import Polynomial; print(Polynomial([1.0, -3.0, 2.0]))"
```

> `uv sync` 会安装 Python 依赖，同时通过 scikit-build-core 编译 nanobind 扩展
> `_core.*.so`（因为 `[tool.uv] package = true`）。
